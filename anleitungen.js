...
function collectYarns() {
  const yarns = [];
  for (let n = 1; n <= yarnN; n++) {
    if (!document.getElementById(`yarn-block-${n}`)) continue;
    yarns.push({
...
function renderYarns(yarns) {
  yarnN = 0;
  const container = document.getElementById('yarn-blocks');
...
// Zähler exakt auf aktuelle Reihe setzen
function syncLeseCounter() {
  if (!currentCounters || currentCounters.length === 0) return;
  currentCounters[0].count = leseCurrentRow + 1;
  renderLeseCounters();
  savePatternCounters();
}
...
// renderLeseMarkers → weiter unten definiert

// Strukturierter Lesemodus
// openLesemodusStrukturiert → weiter unten definiert
