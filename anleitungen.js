...
function renderSchnittteilList() {
  ...
  list.innerHTML = parts.map((p, idx) => {
    const { sfB, sfH, targetMass, warning } = calcScaleFactors(p.massKey, +p.baseMassVal||0, sizeKey);
    const isScaled = Math.abs(sfB - 1) > 0.02 && p.massKey !== 'none' && (+p.baseMassVal||0) > 0;
    const rapport = +p.rapport || 0;
    const rapportPlus = +p.rapportPlus || 0;
    const massLabel = SCHNITT_MASS_OPTIONS.find(o => o.key === (p.massKey||'bust'))?.label || 'Brustumfang';

    const schritteHTML = (p.schritte||[]).map((s, si) =>
      renderSchrittRow(s, idx, si, sfB, sfH, rapport, rapportPlus, isScaled)
    ).join('');
    ...
    return `<div class="schnitt-part">
      <div class="schnitt-part-header" onclick="toggleSchnitt(${idx})">
        <div>
          <h4>${p.name}</h4>
          <p style="font-size:0.65rem;color:var(--textFaint);margin-top:0.1rem;">${massLabel}${isScaled&&targetMass?' → '+targetMass+' cm':''}</p>
        </div>
        <div class="flex align-c gap-sm">
          ${isScaled ? `<span style="font-size:0.65rem;color:var(--green);font-family:var(--mono);">×${sfB.toFixed(2)}</span>` : ''}
      ...
      <div class="schnitt-part-body ${p._open?'open':''}" id="schnitt-body-${idx}">
        ${warning ? `<div class="warning-box mb-sm" style="font-size:0.72rem;">⚠️ ${warning}</div>` : ''}

        <!-- Konfiguration -->
        <div class="card" style="padding:0.85rem;margin-bottom:0.75rem;background:var(--bgWarm);">
          <div class="grid-2 mb-sm">
            <div>
              <label class="lbl">Steuerndes Maß</label>
              <select onchange="updateSchnittField(${idx},'massKey',this.value)">
                ${SCHNITT_MASS_OPTIONS.map(o => `<option value="${o.key}" ${(p.massKey||'bust')===o.key?'selected':''}>${o.icon} ${o.label}</option>`).join('')}
              </select>
            </div>
            <div>
              <label class="lbl">${massLabel} Basisgröße (cm)</label>
              <input type="number" inputmode="decimal" value="${p.baseMassVal||''}"
                placeholder="z.B. 93.5"
        ...
        <!-- Schritte -->
        <div id="schritte-${idx}" class="flex-col gap-sm">
          ${schritteHTML}
        </div>

        <!-- Schritt hinzufügen -->
        <div style="margin-top:0.75rem;">
          <label class="lbl">Schritt hinzufügen</label>
          <div style="display:flex;flex-wrap:wrap;gap:0.4rem;margin-top:0.25rem;">
            ${SCHRITT_TYPEN.map(t =>
              `<button class="btn btn-ghost btn-xs" onclick="addSchritt(${idx},'${t.key}')">${t.icon} ${t.label}</button>`
            ).join('')}
          </div>
        </div>
      </div>
...
function leseNext() { if (leseCurrentRow < leseRows.length-1) { leseCurrentRow++; renderLesemodus(); } }
function leseBack() { if (leseCurrentRow > 0) { leseCurrentRow--; renderLesemodus(); } }
function jumpToRow(i) { leseCurrentRow = i; renderLesemodus(); }

// ═══════════════════════════════════════════════════════════════
// STRUKTURIERTER ANLEITUNG-EDITOR mit DSL-Parser
// ═══════════════════════════════════════════════════════════════
...
function renderAnlSchritte() {
  ...
  if (schnittTeil) {
    const { sfB: b, sfH: h } = calcScaleFactors(schnittTeil.massKey, +schnittTeil.baseMassVal||0, sizeKey);
    sfB = b; sfH = h;
  ...
  if (schritte.length === 0) {
    el.innerHTML = `<p class="txt-light" style="text-align:center;padding:1rem;">Noch keine Schritte. Tippe auf + um anzufangen.</p>`;
    return;
  }

  // Generiere Vorschau für alle Schritte zusammen
    ...
    return `<div style="background:var(--surface);border:1px solid var(--border);border-radius:var(--r-md);padding:0.75rem;">
      <div class="flex align-c gap-sm mb-sm">
        ...
        <input type="text"
          ...
          value="${s.code||''}"
          placeholder="z.B. [90M AS] oder [30R Ger.] (Notiz)"
          style="font-family:var(--mono);font-size:0.85rem;border-color:var(--border);"
          oninput="updateAnlSchritt(${s.id},this.value)"
          onchange="renderAnlSchritte()">
...
function renderSchrittVorschau(id, val) {
  // Wird beim Tippen aufgerufen — komplettes Re-render ist ok für mobile
  // Kein Debounce nötig da Schritte kurz sind
  renderAnlSchritte();
}
...
function renderSyntaxChips() {
  ...
  el.innerHTML = SYNTAX_BEFEHLE.map(b =>
    `<button onclick="insertSyntax('${b.code.replace(/'/g,"\\'")}','${b.label}')"
      style="background:rgba(255,255,255,0.08);border:1px solid rgba(255,255,255,0.15);border-radius:20px;padding:0.25rem 0.7rem;font-family:var(--mono);font-size:0.72rem;color:rgba(255,255,255,0.8);cursor:pointer;white-space:nowrap;">
      ${b.code}
    </button>`
  ).join('');
}

function insertSyntax(code, label) {
  // Füge in das aktuell fokussierte oder letzte Schritt-Feld ein
  const inputs = document.querySelectorAll('.anl-schritt-input');
  const lastInput = inputs[inputs.length - 1];
  if (!lastInput) {
    // Kein Feld da → neuen Schritt erstellen
    addAnlSchritt();
    setTimeout(() => {
      const newInputs = document.querySelectorAll('.anl-schritt-input');
      const ni = newInputs[newInputs.length-1];
      if (ni) { ni.value = code; ni.dispatchEvent(new Event('change')); }
    }, 100);
  } else {
