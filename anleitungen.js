...
function filterAnl(btn, filter) {
  document.querySelectorAll('.f-tab-v2, .f-tab').forEach(b => b.classList.remove('active'));
...
function patternRowHTML(p) {
  const thumb = p.images && p.images.length > 0
    ? `<img src="${p.images[0]}" alt="" style="width:100%;height:100%;object-fit:cover;">`
    : `<span style="font-size:1.8rem;">${p.emoji||'🧶'}</span>`;

  const dots = [1,2,3].map(i =>
    `<span class="p2-dot ${i<=(p.diff||2)?'p2-dot-on':'p2-dot-off'}"></span>`
  ).join('');

  const badgeHTML = getBadge(p.status);
  const desc = p.maschen && p.maschen.length
    ? p.maschen.slice(0,2).join(', ')
    : (p.mat || '');

  return `<div class="pattern-row-v2" onclick="openEditor(${p.id})">
    <div class="p2-thumb">${thumb}</div>
    <div class="p2-info">
      ${p.kat ? `<div class="p2-kat">${p.kat}</div>` : ''}
      <div class="p2-title">${p.title}</div>
      ${desc ? `<div class="p2-desc">${desc}</div>` : ''}
      <div class="p2-footer">
...
function openEditor(id) {
  ...
  if (id === null) {
    ...
    // Reset alle DOM-Elemente explizit
    ...
    const yarnWrap = document.getElementById('yarn-blocks') || document.getElementById('yarn-list');
    if (yarnWrap) yarnWrap.innerHTML = '';
    yarnN = 0;
    const schnittList = document.getElementById('schnitt-list');
    if (schnittList) schnittList.innerHTML = '';
    const anlStepEl = document.getElementById('anl-schritte-list');
    if (anlStepEl) anlStepEl.innerHTML = '';
    const imgPreview = document.getElementById('image-previews');
    if (imgPreview) imgPreview.innerHTML = '';
    const tagWrap = document.getElementById('chips-tags');
    if (tagWrap) tagWrap.innerHTML = '';
    const maschenWrap = document.getElementById('chips-maschen');
    if (maschenWrap) maschenWrap.innerHTML = '';
  } else {
    currentPatternId = id;
...
async function savePattern() {
  ...
  const saveBtn = document.querySelector('.editor-header .btn-green');
  if (saveBtn) { saveBtn.textContent = '…'; saveBtn.disabled = true; }

  if (currentPatternId === null) {
    // NEU: in Supabase anlegen
    data._existsInDB = false;
    const saved = await savePatternToSupabase(data);
    currentPatternId = saved.id;
    data.id = saved.id;
    data._existsInDB = true;
    appState.patterns.unshift(data);
  } else {
    // UPDATE: in Supabase aktualisieren
    const idx = appState.patterns.findIndex(p => p.id === currentPatternId);
    const existing = idx >= 0 ? appState.patterns[idx] : {};
    const merged = { ...existing, ...data, id: currentPatternId, _existsInDB: true };
    const saved = await savePatternToSupabase(merged);
    if (idx >= 0) appState.patterns[idx] = { ...merged, ...saved };
  }

  saveState(); // localStorage als Fallback
...
function addYarn(preset = {}) {
  yarnN++;
  const n = yarnN;
  const div = document.createElement('div');
  div.className = 'card'; div.id = `yarn-block-${n}`;
  div.innerHTML = `
    <div class="flex justify-sb align-c mb-md">
      <label class="lbl" style="margin:0;">Garn ${n}</label>
      <button onclick="document.getElementById('yarn-block-${n}').remove()" style="background:none;border:none;cursor:pointer;color:var(--textLight);">✕</button>
    </div>
    <div class="flex-col gap-sm">
      <div class="grid-2">
        <div><label class="lbl">Garnname</label><input type="text" id="yarn-${n}-name" placeholder="z.B. Merino DK" value="${preset.name||''}"></div>
        <div><label class="lbl">Marke</label><input type="text" id="yarn-${n}-brand" placeholder="z.B. Drops" value="${preset.brand||''}"></div>
      </div>
      <div class="grid-2">
        <div><label class="lbl">Farbe</label><input type="text" id="yarn-${n}-color" placeholder="—" value="${preset.color||''}"></div>
        <div><label class="lbl">Lot-Nr.</label><input type="text" id="yarn-${n}-lot" placeholder="—" value="${preset.lot||''}"></div>
      </div>
      <div><label class="lbl">Garnstärke (CYC)</label>
        <select id="yarn-${n}-cyc">
          <option ${preset.cyc==='0'?'selected':''}>0 – Lace</option>
          <option ${preset.cyc==='1'?'selected':''}>1 – Super Fine</option>
          <option ${preset.cyc==='2'?'selected':''}>2 – Fine</option>
          <option ${!preset.cyc||preset.cyc==='3'?'selected':''}>3 – Light/DK</option>
          <option ${preset.cyc==='4'?'selected':''}>4 – Worsted</option>
          <option ${preset.cyc==='5'?'selected':''}>5 – Bulky</option>
          <option ${preset.cyc==='6'?'selected':''}>6 – Super Bulky</option>
          <option ${preset.cyc==='7'?'selected':''}>7 – Jumbo</option>
        </select>
      </div>
      <div class="grid-3">
        <div><label class="lbl">g/Knäuel</label><input type="number" id="yarn-${n}-g" placeholder="100" value="${preset.g||''}" style="font-family:var(--mono);"></div>
        <div><label class="lbl">Lauflänge m</label><input type="number" id="yarn-${n}-lauf" placeholder="200" value="${preset.lauf||''}" style="font-family:var(--mono);"></div>
        <div><label class="lbl">Anzahl</label><input type="number" id="yarn-${n}-anzahl" placeholder="3" value="${preset.anzahl||''}" style="font-family:var(--mono);"></div>
      </div>
      <div class="grid-2">
        <div><label class="lbl">Material</label><input type="text" id="yarn-${n}-material" placeholder="100% Merino" value="${preset.material||''}"></div>
        <div><label class="lbl">€/Knäuel</label><input type="number" id="yarn-${n}-preis" step="0.01" placeholder="8,90" value="${preset.preis||''}" style="font-family:var(--mono);"></div>
      </div>
    </div>`;
  const container = document.getElementById('yarn-blocks') || document.getElementById('yarn-list');
...
function collectYarns() {
  const yarns = [];
  let n = 1;
  while (document.getElementById(`yarn-block-${n}`)) {
    yarns.push({
      name:     document.getElementById(`yarn-${n}-name`)?.value   || '',
      brand:    document.getElementById(`yarn-${n}-brand`)?.value  || '',
      color:    document.getElementById(`yarn-${n}-color`)?.value  || '',
      lot:      document.getElementById(`yarn-${n}-lot`)?.value    || '',
      cyc:      document.getElementById(`yarn-${n}-cyc`)?.value    || '',
      g:        document.getElementById(`yarn-${n}-g`)?.value      || '',
      lauf:     document.getElementById(`yarn-${n}-lauf`)?.value   || '',
      anzahl:   document.getElementById(`yarn-${n}-anzahl`)?.value || '',
      material: document.getElementById(`yarn-${n}-material`)?.value || '',
      preis:    document.getElementById(`yarn-${n}-preis`)?.value  || '',
    });
    n++;
  }
  return yarns;
...
function leseNext() {
  if (leseCurrentRow < leseRows.length-1) {
    leseCurrentRow++;
    // Auto-increment ersten Zähler
    autoIncrementLeseCounter();
    renderLesemodus();
    savePatternCounters();
  }
}
...
function jumpToRow(i) {
  const prev = leseCurrentRow;
  leseCurrentRow = i;
  // Auto-increment wenn vorwärts
  if (i > prev) autoIncrementLeseCounter();
  renderLesemodus();
  savePatternCounters();
}

function autoIncrementLeseCounter() {
  if (!currentCounters || currentCounters.length === 0) return;
  // Ersten Zähler hochzählen
  currentCounters[0].count++;
  renderLeseCounters();
}

function renderLeseCounters() {
  const el = document.getElementById('lese-counters');
  ...
  inner.innerHTML = currentCounters.map(c => `
    <div style="display:flex;align-items:center;gap:0.5rem;background:var(--surface);border:1px solid var(--border);border-radius:var(--r-md);padding:0.4rem 0.75rem;flex-shrink:0;">
      <span style="font-size:0.72rem;color:var(--textLight);font-weight:600;">${c.label}</span>
      <button onclick="leseCounterDec(${c.id})" style="background:var(--border);border:none;border-radius:50%;width:24px;height:24px;cursor:pointer;font-size:0.9rem;">−</button>
      <span style="font-family:var(--mono);font-size:1rem;font-weight:700;color:var(--green);min-width:28px;text-align:center;">${c.count}</span>
      <button onclick="leseCounterInc(${c.id})" style="background:var(--green);color:white;border:none;border-radius:50%;width:24px;height:24px;cursor:pointer;font-size:0.9rem;">+</button>
      ${c.target ? `<span style="font-size:0.68rem;color:var(--textFaint);font-family:var(--mono);">/ ${c.target}</span>` : ''}
    </div>`).join('');
}

function leseCounterInc(id) {
  const c = currentCounters.find(x => x.id === id);
  if (c) { c.count++; renderLeseCounters(); savePatternCounters(); }
}
function leseCounterDec(id) {
  const c = currentCounters.find(x => x.id === id);
  if (c && c.count > 0) { c.count--; renderLeseCounters(); savePatternCounters(); }
}
...
function savePatternCounters() {
  ...
  const idx = appState.patterns.findIndex(p => p.id === currentPatternId);
  if (idx >= 0) {
    appState.patterns[idx].counters = JSON.parse(JSON.stringify(currentCounters));
    saveState();
  }
}

function renderLeseMarkers() {
  const hint  = document.getElementById('lese-markers-hint');
  const inner = document.getElementById('lese-markers-inner');
  if (!hint || !inner) return;

  const aktuelleReihe = leseCurrentRow + 1;
  const relevant = (currentMarkers || []).filter(m => m.row === aktuelleReihe);

  if (relevant.length === 0) { hint.style.display = 'none'; return; }

  hint.style.display = 'block';
  inner.innerHTML = `<span style="font-size:0.7rem;color:var(--green);font-weight:700;margin-right:0.4rem;">Markierung R${aktuelleReihe}:</span>` +
    relevant.map(m =>
      `<span style="background:var(--greenPale);color:var(--green);border-radius:12px;padding:0.15rem 0.5rem;font-size:0.72rem;font-weight:600;">
        M${m.stitch} ${markerTypeLabel(m.type)}${m.note?' — '+m.note:''}
      </span>`
    ).join('');
}

// Strukturierter Lesemodus → siehe openLesemodusStrukturiert() weiter unten
...
function skalierteAusgabe(codeRaw, sfB, sfH, rapport, rapportPlus, laufendeMaschen) {
  ...
  if (/^(\d+)M Abn\. bds\. \/ (\d+)R \/ (\d+)x$/.test(c)) {
    ...
    return { text: `${sM} Maschen abnehmen je Seite — ${mf.text||mf}`, delta: -(sM * 2 * wdh) };
  }
  if (/^(\d+)M Abn\. eins\. \/ (\d+)R \/ (\d+)x$/.test(c)) {
    ...
    const wdh = nums[2];
    const mf  = magicFormula(sR, wdh);
    return { text: `${sM} Maschen abnehmen — ${mf.text||mf}`, delta: -(sM * wdh) };
  }
  if (/^(\d+)M Zun\. bds\. \/ (\d+)R \/ (\d+)x$/.test(c)) {
    ...
    const wdh = nums[2];
    const mf  = magicFormula(sR, wdh);
    return { text: `${sM} Maschen zunehmen je Seite — ${mf.text||mf}`, delta: sM * 2 * wdh };
  }
  if (/^(\d+)M Zun\. eins\. \/ (\d+)R \/ (\d+)x$/.test(c)) {
    ...
    const wdh = nums[2];
    const mf  = magicFormula(sR, wdh);
    return { text: `${sM} Maschen zunehmen — ${mf.text||mf}`, delta: sM * wdh };
  }
  // Rest abketten
...
function renderAnlSchritte() {
  ...
  el.innerHTML = schritte.map((s, i) => {
    ...
    return `<div class="anl-schritt-drag"
      draggable="true"
      data-idx="${i}"
      ...
      style="background:var(--surface);border:1px solid var(--border);border-radius:var(--r-md);padding:0.75rem;cursor:grab;">
      <div class="flex align-c gap-sm mb-sm">
        <span style="color:var(--textFaint);cursor:grab;font-size:1rem;flex-shrink:0;">⠿</span>
        <div class="anl-step-circle">${i+1}</div>
        <input type="text"
          class="anl-schritt-input"
          data-id="${s.id}"
          value="${(s.code||'').replace(/"/g,'&quot;')}"
          placeholder="z.B. [90M AS] oder (Notiz)"
          style="font-family:var(--mono);font-size:0.85rem;border-color:var(--border);flex:1;"
      ...
      ${vorschau && vorschau.text ? `
        <div style="padding-left:2.5rem;">
          <span class="anl-step-result">= ${vorschau.text}${vorschau.maschen ? ` (${vorschau.maschen}M)` : ''}</span>
        </div>` : ''}
    </div>`;
  }).join('');
}
...
function dragOver(e) {
  ...
  // Visuelles Feedback
  document.querySelectorAll('.anl-schritt-drag').forEach(el => {
    el.style.borderColor = 'var(--border)';
  });
  const target = e.currentTarget;
  if (target) target.style.borderColor = 'var(--green)';
}
...
function dragDrop(e, targetIdx) {
  ...
  const key = getAktuellesSchnitteil();
  const schritte = getSchritte(key);
  const moved = schritte.splice(dragSrcIdx, 1)[0];
  schritte.splice(targetIdx, 0, moved);
  dragSrcIdx = null;
  renderAnlSchritte();
}
...
async function deletePattern() {
  ...
  if (!id) { showSnackbar('Erst speichern bevor löschen'); return; }
  const p = appState.patterns.find(x => x.id === id);
  const name = p ? `„${p.title}"` : 'diese Anleitung';
  if (!confirm(`${name} wirklich löschen? Kann nicht rückgängig gemacht werden.`)) return;

  appState.patterns = appState.patterns.filter(x => x.id !== id);
  currentPatternId = null;
  await deletePatternFromSupabase(id);
  goBack();
  setTimeout(() => renderAnleitungen('alle'), 100);
  showSnackbar('Anleitung gelöscht');
}

// ─── LESEMODUS: VOLLEN TEXT ───────────────────────────────────
let leseFulltextMode = false;

function toggleLeseFulltext() {
  leseFulltextMode = !leseFulltextMode;
  const btn = document.getElementById('lese-fulltext-btn');
  if (btn) btn.classList.toggle('active', leseFulltextMode);

  const controls = document.querySelector('.lese-controls');
  const countersEl = document.getElementById('lese-counters');
  if (controls) controls.style.display = leseFulltextMode ? 'none' : '';
  if (countersEl) countersEl.style.display = leseFulltextMode ? 'none' : '';
...
function renderLeseFulltext() {
  const body = document.getElementById('lese-body');
  if (!body) return;

  // Alle Schnittteile der aktuellen Anleitung
  const key = document.getElementById('anl-schnitt-select')?.value || '__allgemein__';
  const sizeKey = document.getElementById('ed-size-select')?.value || 'damen_m';
  const allKeys = ['__allgemein__', ...(window._currentSchnitt||[]).map(s=>s.name)];

  let html = '<div class="lese-fulltext-block">';
  allKeys.forEach(k => {
    const schritte = getSchritte(k);
    if (!schritte || !schritte.length) return;
    const schnittTeil = (window._currentSchnitt||[]).find(s => s.name === k);
    let sfB=1,sfH=1,rapport=0,rapportPlus=0;
    if (schnittTeil) {
      const r = calcScaleFactors(schnittTeil.massKey, +schnittTeil.baseMassVal||0, sizeKey);
      sfB=r.sfB; sfH=r.sfH;
    ...
    const ausgabe = generiereAusgabe(schritte, sfB, sfH, rapport, rapportPlus);
    if (!ausgabe.length) return;
    const label = k === '__allgemein__' ? 'Allgemeine Anleitung' : k;
    html += `<h3>${label}</h3>`;
    ausgabe.forEach(a => {
      if (a.text) html += `<p>${a.nr}. ${a.text}${a.maschen ? ` <span style="color:var(--green);font-weight:600;font-family:var(--mono);font-size:0.85em;">= ${a.maschen}M</span>` : ''}</p>`;
    });
  });
  html += '</div>';
  body.innerHTML = html;
