...
function renderCounters() {
  ...
  el.innerHTML = currentCounters.map(c => `
    <div class="card" style="padding:0.75rem;">
      <div class="flex justify-sb align-c mb-sm">
        <input type="text" value="${c.label}" style="border:none;background:transparent;font-family:var(--serif);font-weight:600;font-size:0.9rem;color:var(--text);padding:0;width:auto;flex:1;" oninput="updateCounterLabel(${c.id},this.value)">
        <div class="flex align-c gap-sm">
          ${c.target ? `<span style="font-family:var(--mono);font-size:0.72rem;color:var(--textLight);">/ ${c.target}</span>` : ''}
          <button onclick="removeCounter(${c.id})" style="background:none;border:none;cursor:pointer;color:var(--textFaint);font-size:0.85rem;">✕</button>
        </div>
      </div>
      <div class="counter-display" style="padding:0.5rem 0;">
        <span class="counter-number">${c.count}</span>
        ${c.target ? `<div style="height:4px;background:var(--border);border-radius:2px;margin-top:0.5rem;"><div style="height:100%;background:var(--terra);border-radius:2px;width:${Math.min(100,(c.count/c.target)*100)}%;transition:width 0.2s;"></div></div>` : ''}
      </div>
      <div class="counter-btns">
        <button class="counter-big-btn counter-dec" onclick="counterDec(${c.id})">−</button>
        <button onclick="counterReset(${c.id})" style="background:var(--bgWarm);border:none;cursor:pointer;border-radius:var(--r-sm);padding:0.4rem 0.85rem;font-size:0.75rem;color:var(--textLight);">Reset</button>
        <button class="counter-big-btn counter-inc" onclick="counterInc(${c.id})">+</button>
      </div>
      <div class="mt-sm flex align-c gap-sm">
        <label class="lbl" style="margin:0;white-space:nowrap;">Ziel:</label>
        <input type="number" value="${c.target||''}" placeholder="optional" style="font-family:var(--mono);font-size:0.8rem;" oninput="updateCounterTarget(${c.id},this.value)">
      </div>
    </div>`).join('');
}
...
function addMarker() {
  // Felder leeren
  ['marker-row','marker-st','marker-note'].forEach(id => {
    document.getElementById(id).value = '';
  });
  document.getElementById('marker-type').value = 'raglan';
  openSheet('marker');
}

function saveMarker() {
  const row  = +document.getElementById('marker-row').value || 0;
  const st   = +document.getElementById('marker-st').value  || 0;
  const type = document.getElementById('marker-type').value;
  const note = document.getElementById('marker-note').value.trim();

  currentMarkers.push({ id: Date.now(), row, stitch: st, type, note });
...
function renderMarkers() {
  ...
  el.innerHTML = currentMarkers.map(m => `
    <div class="marker-row">
      <div class="marker-type-dot" style="background:${MARKER_COLORS[m.type]||'#9c7b5a'};"></div>
      <div class="marker-info">
        <div class="marker-note">${m.note || m.type}</div>
        <div class="marker-pos">R${m.row} · M${m.stitch} · ${markerTypeLabel(m.type)}</div>
      </div>
      <button onclick="removeMarker(${m.id})" style="background:none;border:none;cursor:pointer;color:var(--textFaint);font-size:0.85rem;padding:0.2rem;">✕</button>
    </div>`).join('');
}
