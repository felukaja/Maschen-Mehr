...
// ─── SCHNITTTEIL & SKALIERUNGS-ENGINE ────────────────────────

// Welches Körpermaß steuert welchen Schnittteil-Typ
...
const BODY_MEAS_FULL = {
  ...
  herren_3xl:{bust:139.5,hip:142.5,waist:124.5,sleeve:52,back:47, shoulder:47, armhole:24.5,head:60,foot:29.5,ankle:27.5,inseam:84,thigh:73},
  herren_4xl:{bust:149.5,hip:152.5,waist:134.5,sleeve:52.5,back:48,shoulder:49,armhole:25,  head:61,foot:30,ankle:28.5,inseam:84,thigh:77},
  ...
  kind_14j: {bust:80,hip:86,waist:66,sleeve:50,back:43,shoulder:32,armhole:16.5,head:53,foot:23,ankle:20,inseam:70,thigh:50},
  kind_16j: {bust:86,hip:92,waist:69,sleeve:52,back:44.5,shoulder:34,armhole:17.5,head:54,foot:24,ankle:21,inseam:74,thigh:54},
...
function calcScaleFactors(massKey, baseMassVal, targetSizeKey) {
  if (!massKey || massKey === 'none' || !baseMassVal || baseMassVal <= 0) {
    return { sfBreite:1, sfHoehe:1, targetMass:null, warning:null };
  }
  const target = BODY_MEAS_FULL[targetSizeKey];
  if (!target) return { sfBreite:1, sfHoehe:1, targetMass:null, warning:null };

  const targetMass = target[massKey];
  if (!targetMass) return { sfBreite:1, sfHoehe:1, targetMass:null, warning:null };

  const sfBreite = targetMass / baseMassVal;
  // Längenmaße steuern auch die Höhe direkt; Umfangsmaße → Höhe proportional
  const sfHoehe = sfBreite;

  let warning = null;
  if (sfBreite < 0.7 || sfBreite > 1.3) warning = `Großer Größensprung (×${sfBreite.toFixed(2)}) — Konstruktion prüfen`;

  return { sfBreite, sfHoehe, targetMass, warning };
}

function roundToRapport(raw, mult, plus) {
  if (!mult || mult <= 1) return Math.round(raw);
  plus = plus || 0;
...
function addSchnittFromSheet(name) {
  ...
  const defaultMass = SCHNITT_DEFAULT_MASS[name] || 'bust';
  // Vorbelegen mit baseBust falls Brust das steuernde Maß
  const baseBust = +document.getElementById('ed-base-bust')?.value || '';
  window._currentSchnitt.push({
    name, maschen:'', reihen:'', breite:'', laenge:'', abnahmen:'',
    rapport:'', rapportPlus:'',
    massKey: defaultMass,
    baseMassVal: defaultMass === 'bust' ? baseBust : '',
    _open: true,
  });
  renderSchnittteilList();
}
...
function renderSchnittteilList() {
  ...
  list.innerHTML = parts.map((p, idx) => {
    ...
    const { sfBreite, sfHoehe, targetMass, warning } =
      calcScaleFactors(p.massKey, baseMassVal, sizeKey);

    const isScaled = Math.abs(sfBreite - 1) > 0.02 && p.massKey !== 'none' && baseMassVal > 0;
    const rapport     = +p.rapport || 0;
    const rapportPlus = +p.rapportPlus || 0;

    const scaledM = (p.maschen && isScaled) ? roundToRapport(+p.maschen * sfBreite, rapport, rapportPlus) : null;
    const scaledR = (p.reihen  && isScaled) ? Math.round(+p.reihen * sfHoehe) : null;

    const massOpt = SCHNITT_MASS_OPTIONS.find(o => o.key === (p.massKey||'bust'));
    const massLabel = massOpt?.label || 'Brustumfang';
    ...
    return `<div class="schnitt-part">
      <div class="schnitt-part-header" onclick="toggleSchnitt(${idx})">
        <div>
          ...
          <p style="font-size:0.65rem;color:var(--textFaint);margin-top:0.1rem;">
            ${massLabel}${isScaled ? ` · Ziel: ${targetMass} cm · ×${sfBreite.toFixed(3)}` : ''}
          </p>
        </div>
        <div class="flex align-c gap-sm">
          ${isScaled && scaledM !== null ? `<span style="font-family:var(--mono);font-size:0.78rem;color:var(--green);font-weight:700;">${scaledM}M</span>` : ''}
      ...
      <div class="schnitt-part-body ${p._open?'open':''}" id="schnitt-body-${idx}">
        ...
        ${isScaled ? `<div style="background:var(--greenFaint);border:1px solid var(--greenPale);border-radius:var(--r-sm);padding:0.5rem 0.75rem;font-size:0.72rem;margin-bottom:0.75rem;color:var(--greenDark);">
          ✓ Skaliert · Breite ×${sfBreite.toFixed(3)} · Höhe ×${sfHoehe.toFixed(3)}
        </div>` : ''}

        <div class="mb-sm">
          <label class="lbl">Steuerndes Maß</label>
          <select onchange="updateSchnitt(${idx},'massKey',this.value)">
            ${SCHNITT_MASS_OPTIONS.map(o =>
              `<option value="${o.key}" ${(p.massKey||'bust')===o.key?'selected':''}>${o.icon} ${o.label}</option>`
            ).join('')}
          </select>
        </div>

        <div class="mb-sm">
          <label class="lbl">${massLabel} in deiner Basisgröße (cm)</label>
          <input type="number" inputmode="decimal"
            value="${p.baseMassVal||''}"
            placeholder="z.B. ${massOpt?.key==='bust'?'93.5':massOpt?.key==='hip'?'101.5':''}"
            style="font-family:var(--mono);"
            onchange="updateSchnitt(${idx},'baseMassVal',this.value)">
          <p style="font-size:0.68rem;color:var(--textFaint);margin-top:0.2rem;">Körpermaß (ohne Ease) der Größe für die du diese Anleitung geschrieben hast</p>
        </div>

        <div class="grid-2 mb-sm">
          <div>
            <label class="lbl">Maschen Original</label>
            <input type="number" inputmode="numeric" value="${p.maschen||''}" placeholder="0"
              style="font-family:var(--mono);font-weight:700;"
              onchange="updateSchnitt(${idx},'maschen',this.value)">
            ${isScaled && scaledM !== null ? `<div style="margin-top:0.3rem;padding:0.3rem 0.5rem;background:var(--greenPale);border-radius:4px;font-family:var(--mono);font-size:0.82rem;color:var(--green);font-weight:700;">→ ${scaledM} M</div>` : ''}
          </div>
          <div>
            <label class="lbl">Reihen Original</label>
            <input type="number" inputmode="numeric" value="${p.reihen||''}" placeholder="0"
              style="font-family:var(--mono);font-weight:700;"
              onchange="updateSchnitt(${idx},'reihen',this.value)">
            ${isScaled && scaledR !== null ? `<div style="margin-top:0.3rem;padding:0.3rem 0.5rem;background:var(--greenPale);border-radius:4px;font-family:var(--mono);font-size:0.82rem;color:var(--green);font-weight:700;">→ ${scaledR} R</div>` : ''}
          </div>
        </div>
        ...
        <div class="grid-2 mb-sm">
          <div>
            ...
            <input type="number" inputmode="numeric" value="${p.rapport||''}" placeholder="z.B. 6"
              style="font-family:var(--mono);" onchange="updateSchnitt(${idx},'rapport',this.value)">
          </div>
          <div>
            <label class="lbl">Plus</label>
            <input type="number" inputmode="numeric" value="${p.rapportPlus||''}" placeholder="z.B. 1"
              style="font-family:var(--mono);" onchange="updateSchnitt(${idx},'rapportPlus',this.value)">
          </div>
        </div>
        <p style="font-size:0.68rem;color:var(--textFaint);margin-top:-0.4rem;margin-bottom:0.75rem;">Shell=6+1 · Granny=4+0 · Leer lassen wenn kein Rapport</p>

        <div class="grid-2 mb-sm">
          <div><label class="lbl">Breite Original (cm)</label>
            <input type="number" inputmode="decimal" value="${p.breite||''}" placeholder="—"
              style="font-family:var(--mono);" onchange="updateSchnitt(${idx},'breite',this.value)">
            ${isScaled && p.breite ? `<div style="margin-top:0.3rem;font-family:var(--mono);font-size:0.75rem;color:var(--green);">→ ${(+p.breite * sfBreite).toFixed(1)} cm</div>` : ''}
          </div>
          <div><label class="lbl">Länge Original (cm)</label>
            <input type="number" inputmode="decimal" value="${p.laenge||''}" placeholder="—"
              style="font-family:var(--mono);" onchange="updateSchnitt(${idx},'laenge',this.value)">
            ${isScaled && p.laenge ? `<div style="margin-top:0.3rem;font-family:var(--mono);font-size:0.75rem;color:var(--green);">→ ${(+p.laenge * sfHoehe).toFixed(1)} cm</div>` : ''}
          </div>
        </div>

        <div><label class="lbl">Abnahmen / Zunahmen</label>
          <input type="text" value="${p.abnahmen||''}" placeholder="z.B. alle 6 R 1 Abnahme ×8"
            onchange="updateSchnitt(${idx},'abnahmen',this.value)">
        </div>
      </div>
    </div>`;
  }).join('');
