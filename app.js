...
function renderDashPatterns() {
  ...
  if (!el) return;
  // Zeige die letzten 3 bearbeiteten Anleitungen (nicht archiviert)
  const active = [...appState.patterns]
    .filter(p => p.status !== 'arch')
    .slice(0, 3);
  if (active.length === 0) {
    el.innerHTML = '<p class="txt-light" style="text-align:center;padding:1rem;">Noch keine Anleitungen.</p>';
    return;
  }
...
function openGarnImRechner(n) {
  ...
  // Werte in sessionStorage zwischenspeichern
  sessionStorage.setItem('garnRechner', JSON.stringify({ lauf, g, anzahl, mat }));
  navTo('calc-garn');
  setTimeout(() => {
    if (lauf) {
      const elLauf = document.getElementById('garn-ol');
      if (elLauf) elLauf.value = lauf;
    }
    if (g) {
      const elG = document.getElementById('garn-og');
      if (elG) elG.value = g;
    }
    if (anzahl) {
      const elA = document.getElementById('garn-ok');
      if (elA) elA.value = anzahl;
    }
    if (mat) {
      const elM = document.getElementById('garn-material-orig');
      if (elM) elM.value = mat;
    }
    calcGarn();
