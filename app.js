...
function renderDashPatterns() {
  ...
  if (!el) return;
  const active = appState.patterns.filter(p => p.status === 'open' || p.status === 'draft').slice(0, 3);
  if (active.length === 0) {
    el.innerHTML = '<p class="txt-light" style="text-align:center;padding:1rem;">Noch keine aktiven Anleitungen.</p>';
    return;
  }
  el.innerHTML = active.map(p => patternRowHTML(p)).join('');
}
...
function getBadge(s) {
  if (s==='draft') return '<span class="badge badge-draft">Entwurf</span>';
...
function todoRowHTML(t, mode) {
  ...
  return `<div class="todo-row" id="todo-row-${t.id}-${mode}">
    ...
    <button onclick="deleteTodo(${t.id},'${mode}')" style="background:none;border:none;cursor:pointer;color:var(--textFaint);font-size:1rem;padding:0.2rem;">🗑</button>
  </div>`;
}
...
function addTodo() {
  ...
  appState.todos.unshift({ id: appState.nextTodoId++, text, date: date||'', cat, done: false });
...
function animateRemoveTodo(id, mode, cb) {
  const row = document.getElementById(`todo-row-${id}-${mode}`);
  if (row) {
...
function loadProfilFields() {
  ...
  ['name','groesse','brust','taille','hufte','hoehe','kopf','fuss'].forEach(k => {
