...
let appState = {
  ...
  todos: [],
  notizen: [],
  profil: { name:'', groesse:'damen_m', brust:'', taille:'', hufte:'', hoehe:'', kopf:'', fuss:'' },
...
function goBack() {
  // Timer stoppen wenn Lesemodus verlassen wird
  if (currentScreen === 'lesemodus' && typeof stopLeseTimer === 'function') {
    stopLeseTimer();
  }
...
function dbToPattern(r) {
  return {
    ...
    abbr:             r.abbr             || '',
    hoursTotal:       r.hours_total      || 0,
    hoursLog:         r.hours_log        || [],
    _existsInDB:      true,
  };
}
...
function patternToDB(p) {
  return {
    ...
    abbr:               p.abbr             || '',
    hours_total:        p.hoursTotal       || 0,
    hours_log:          p.hoursLog         || [],
  };
}
...
async function saveProfilToSupabase() {
  ...
  const data = {
    user_id: USER_ID,
    name: p.name||'', groesse: p.groesse||'damen_m',
...
function updateGreeting() {
  ...
  const name = appState.profil.name || 'Neuankömmling';
...
// ─── PROFIL ───────────────────────────────────────────────────
function saveProfil() {
  appState.profil.name    = document.getElementById('profil-name').value.trim() || '';
...
async function deleteProfil() {
  if (!confirm('Profil wirklich löschen? Name, Maße und Einstellungen werden zurückgesetzt.')) return;
  appState.profil   = { name:'', groesse:'damen_m', brust:'', taille:'', hufte:'', hoehe:'', kopf:'', fuss:'' };
  appState.settings = { unit:'cm', yarnSys:'cyc' };
  try { await supaFetch(`profil?user_id=eq.${USER_ID}`, 'DELETE'); } catch(e) {}
  loadProfilFields();
  updateGreeting();
  showSnackbar('Profil gelöscht');
  goBack();
}
