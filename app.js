...
// ─── INIT ─────────────────────────────────────────────────────
// Rechner-Screens werden via rechner.js befüllt
['calc-mp','calc-groesse','calc-mass','calc-garn','calc-nadel','calc-staerke'].forEach(() => {});

// ─── PROFIL LÖSCHEN ──────────────────────────────────────────
async function deleteProfil() {
  if (!confirm('Profil wirklich löschen? Name, Maße und Einstellungen werden zurückgesetzt. Anleitungen bleiben erhalten.')) return;
  appState.profil   = { name:'', groesse:'damen_m', brust:'', taille:'', hufte:'', hoehe:'', kopf:'', fuss:'' };
  appState.settings = { unit:'cm', yarnSys:'cyc' };
  try {
    await supaFetch(`profil?user_id=eq.${USER_ID}`, 'DELETE');
  } catch(e) {}
  loadProfilFields();
  updateGreeting();
