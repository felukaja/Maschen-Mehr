...
const EASE_DEFAULTS = {
  'Pullover (klassisch)':  {bust:10,waist:7, hip:8},
  'Pullover (oversized)':  {bust:20,waist:18,hip:15},
  'Cardigan':              {bust:12,waist:9, hip:10},
  'Jacke':                 {bust:15,waist:12,hip:12},
  'Top / Crop':            {bust:4, waist:2, hip:0},
  'Kleid (klassisch)':     {bust:8, waist:8, hip:12},
  'Hose':                  {bust:0, waist:5, hip:8},
  'Rock':                  {bust:0, waist:5, hip:8},
  'Mütze':                 {bust:-3,waist:0, hip:0},
  'Socken':                {bust:-3,waist:0, hip:0},
  'Handschuhe':            {bust:0, waist:0, hip:0},
  'Baby-Jäckchen':         {bust:5, waist:3, hip:3},
};
...
function calcMass() {
  ...
  if (brust > 0) {
    ...
    if (kleidung && EASE_DEFAULTS[kleidung] && easeEl) {
      ...
      easeEl.innerHTML = `<div style="background:var(--greenFaint);border:1px solid var(--greenPale);border-radius:var(--r-sm);padding:0.65rem;">
        <p class="lbl">Empfohlener Ease (Häkeln +2–5 cm mehr als Stricken)</p>
        <p style="font-family:var(--mono);color:var(--green);font-size:0.85rem;">Brust: +${e.bust} cm → <strong>${brust+e.bust}</strong> cm Fertigmaß</p>
        ${taille>0&&e.waist ? `<p style="font-family:var(--mono);font-size:0.82rem;color:var(--green);">Taille: +${e.waist} cm → ${taille+e.waist} cm</p>` : ''}
        ${hufte>0&&e.hip ? `<p style="font-family:var(--mono);font-size:0.82rem;color:var(--green);">Hüfte: +${e.hip} cm → ${hufte+e.hip} cm</p>` : ''}
      </div>`;
    }
  } else {
