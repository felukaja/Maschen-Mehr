/* ═══════════════════════════════════════════════════════════════
   MASCHEN & MASSE — rechner.js v6.0
   + Raglan-Passen-Rechner (empirisch aus 6 echten Anleitungen)
   ═══════════════════════════════════════════════════════════════ */

'use strict';

// ─── RAGLAN k-FAKTOREN ────────────────────────────────────────
// Empirisch ermittelt aus 6 Hobbii-Anleitungen + Projektdaten
// k = effektive Passen-Höhe / theoretische Reihenhöhe
// Basiert auf Stichhöhenverhältnissen sc:hStb:Stb = 1.0:1.6:2.4
// (aus Projektdatei Häkel-Maschen-Lexikon, Abschnitt 5.1)
//
// Belege:
//   hStb top-down  k=1.32 → Flieder Pulloverkleid (18M/14R), 4/4 Größen ±0.5cm
//   erwfM top-down k=1.03 → Finisterre (17M/14R), 4/4 Größen ±1.0cm
//   hStb bottom-up k=1.08 → Katia Raglan (14M/8R), ±0.3cm
//   Stb top-down   k=1.65 → theoretisch (H:B-Verhältnis × baseline)
//   fM  top-down   k=0.83 → theoretisch
const RAGLAN_K = {
  // [maschenart][richtung]
  'fM (sc)':        { 'top-down': 0.83, 'bottom-up': 0.96 },
  'Halbe Stäbchen': { 'top-down': 1.32, 'bottom-up': 1.08 },
  'erwfM':          { 'top-down': 1.03, 'bottom-up': 1.01 },
  'Stäbchen (dc)':  { 'top-down': 1.65, 'bottom-up': 1.16 },
  'Doppelstäbchen': { 'top-down': 2.20, 'bottom-up': 1.30 },
};
...
// ── MASCHENPROBE — Probe ist primär, Material nur als Hinweis ─
  ...
  el.innerHTML = `
    ...
    <div class="scroll-body"><div class="p-main flex-col gap-lg">
      ...
      <div class="card card-pale-green">
        <p class="txt-light"><strong style="color:var(--green);">Die Probe ist immer die Wahrheit.</strong> Häkle mind. 15×15 cm, wasche und blocke das Stück, dann messe — erst danach ist die Probe verlässlich.</p>
      </div>
      ...
      <div id="mp-res" class="result-box"><label class="lbl">Ergebnis</label><p class="txt-light">Werte eingeben →</p></div>
      ...
      <!-- Material als Info, kein Korrekturfaktor -->
      <div class="card">
        <label class="lbl">Material-Hinweise (optional)</label>
        <select id="mp-material" onchange="calcMP()">
          <option value="">— Material wählen für Hinweise —</option>
          ${Object.keys(MATERIAL_FAKTOREN).map(m => `<option value="${m}">${m}</option>`).join('')}
        </select>
        <div id="mp-material-note" class="mt-sm"></div>
      </div>
...
function calcMP() {
  ...
  // Material-Hinweis anzeigen
  const noteEl = document.getElementById('mp-material-note');
  if (noteEl && matKey && MATERIAL_FAKTOREN[matKey]) {
    const f = MATERIAL_FAKTOREN[matKey];
    noteEl.innerHTML = `<div class="warning-box" style="background:var(--greenFaint);border-color:var(--greenPale);">
      <p style="font-size:0.78rem;color:var(--greenDark);">${f.note}</p>
      ${(f.hoehe !== 1) ? `<p style="font-size:0.72rem;color:var(--textLight);margin-top:0.2rem;">⚠ Dieses Material verändert die Länge — Probe nach Waschen neu messen!</p>` : ''}
    </div>`;
  } else if (noteEl) {
    noteEl.innerHTML = '';
  }
  ...
  if (m > 0 && r > 0) {
    // Probe ist die Wahrheit — keine Korrekturfaktoren
    const mpc = m / w;
    const rpc = r / h;

    el.innerHTML = `<label class="lbl">Deine Gauge</label>
      <div class="grid-2 mt-sm">
        <div><p class="result-num">${(mpc*10).toFixed(2)}</p><p class="result-unit">Maschen / 10 cm</p></div>
        <div><p class="result-num">${(rpc*10).toFixed(2)}</p><p class="result-unit">Reihen / 10 cm</p></div>
      </div>
      <p class="txt-light mt-sm" style="font-family:var(--mono);font-size:0.75rem;">1 M = ${(1/mpc).toFixed(2)} cm · 1 R = ${(1/rpc).toFixed(2)} cm</p>`;
    ...
    if ((tw > 0 || th > 0) && tres) {
      ...
      if (tw > 0) {
        ...
        html += `<p style="font-family:var(--mono);color:var(--green);font-size:0.95rem;font-weight:700;">${adjM} Maschen</p>
      ...
      if (th > 0) {
        ...
        html += `<p style="font-family:var(--mono);color:var(--green);font-size:0.95rem;font-weight:700;margin-top:0.5rem;">${adjR} Reihen</p>
...
function renderCalcGroesse() {
  ...
  el.innerHTML = `
    ...
    <div class="scroll-body"><div class="p-main flex-col gap-lg">
      ...
      <div class="card card-pale-green">
        <p class="txt-light"><strong style="color:var(--green);">So funktioniert es:</strong> Trage die Gauge der Anleitung und deine eigene Gauge ein. Wenn du ein anderes Garn oder eine andere Maschenart verwendest als die Anleitung, werden die Korrekturfaktoren eingerechnet — SF_Breite und SF_Höhe werden immer separat berechnet.</p>
      </div>
      ...
      <div class="card card-terra">
        ...
        <div class="mt-sm">
          <label class="lbl">Material des Original-Garns (aus der Anleitung)</label>
          <select id="gr-material-orig" onchange="calcGR()">
            <option value="">— falls bekannt —</option>
            ${Object.keys(MATERIAL_FAKTOREN).map(m => `<option value="${m}">${m}</option>`).join('')}
          </select>
          <p id="gr-orig-note" class="txt-light mt-sm" style="font-size:0.72rem;"></p>
        </div>
        <div class="mt-sm">
          <label class="lbl">Hauptmaschenart der Anleitung (Original)</label>
          <select id="gr-maschen-orig" onchange="calcGR()">
            <option value="">— falls bekannt —</option>
      ...
      <div class="card card-green">
        ...
        <div class="mt-sm">
          <label class="lbl">Dein Material</label>
          <select id="gr-material" onchange="calcGR()">
            <option value="">— falls anderes Material als Original —</option>
            ${Object.keys(MATERIAL_FAKTOREN).map(m => `<option value="${m}">${m}</option>`).join('')}
          </select>
          <p id="gr-mat-note" class="txt-light mt-sm" style="font-size:0.72rem;"></p>
        </div>
        <div class="mt-sm">
          <label class="lbl">Deine Hauptmaschenart</label>
          <select id="gr-maschen" onchange="calcGR()">
            <option value="">— falls andere Maschenart als Original —</option>
            ${Object.keys(MASCHEN_FAKTOREN).map(m => `<option value="${m}">${m}</option>`).join('')}
          </select>
...
function calcGR() {
  ...
  const matOrigKey  = document.getElementById('gr-material-orig')?.value;
  const masOrigKey  = document.getElementById('gr-maschen-orig')?.value;
  const matKey      = document.getElementById('gr-material')?.value;
  const masKey      = document.getElementById('gr-maschen')?.value;

  const matOrigF = (matOrigKey && MATERIAL_FAKTOREN[matOrigKey]) || {breite:1,hoehe:1,note:''};
  const masOrigF = (masOrigKey && MASCHEN_FAKTOREN[masOrigKey])  || {breite:1,hoehe:1,note:''};
  const matF     = (matKey && MATERIAL_FAKTOREN[matKey])         || {breite:1,hoehe:1,note:''};
  const masF     = (masKey && MASCHEN_FAKTOREN[masKey])          || {breite:1,hoehe:1,note:''};

  // Hinweise
  const origNoteEl = document.getElementById('gr-orig-note');
  const matNoteEl  = document.getElementById('gr-mat-note');
  const masNoteEl  = document.getElementById('gr-mas-note');
  if (origNoteEl) origNoteEl.textContent = matOrigF.note;
  if (matNoteEl)  matNoteEl.textContent  = matF.note;
  if (masNoteEl)  masNoteEl.textContent  = masF.note;
  ...
  if (om > 0 && or_ > 0 && nm > 0 && nr > 0) {
    ...
    // SF: dein Material/Maschen ÷ Original-Material/Maschen
    const sfB = sfB_raw * (matF.breite / matOrigF.breite) * (masF.breite / masOrigF.breite);
    const sfH = sfH_raw * (matF.hoehe  / matOrigF.hoehe)  * (masF.hoehe  / masOrigF.hoehe);

    let warn = '';
    if (sfB < 0.7 || sfB > 1.3) warn += '<div class="warning-box mt-sm">SF Breite > 30% — Neukonstruktion empfohlen</div>';
    if (sfH < 0.7 || sfH > 1.3) warn += '<div class="warning-box mt-sm">SF Höhe > 30% — Reihenzahl kritisch prüfen</div>';

    if (os > 0 && rw > 0) {
      const adjM = roundToRapportCalc(os * sfB, mult, plus);
      const adjR = Math.round(rw * sfH);
      const abwCm = Math.abs(adjM - os * sfB) / (nm/10);

      el.innerHTML = `<label class="lbl">Deine Werte</label>
        <div class="grid-2 mt-sm">
          <div><p class="result-num">${adjM}</p><p class="result-unit">Maschen${mult>1?' (Rapport ✓)':''}</p></div>
          <div><p class="result-num">${adjR}</p><p class="result-unit">Reihen</p></div>
        </div>
        <div class="mt-sm" style="background:var(--bgWarm);border-radius:var(--r-sm);padding:0.65rem;">
          <p style="font-size:0.72rem;color:var(--textLight);font-family:var(--mono);">SF Breite: ×${sfB.toFixed(3)} · SF Höhe: ×${sfH.toFixed(3)}</p>
    ...
    <div class="scroll-body"><div class="p-main flex-col gap-lg">
      <div class="card card-terra">
        <div class="flex justify-sb align-c mb-sm">
          <label class="lbl" style="margin:0;">Deine Körpermaße</label>
          <button class="btn btn-green btn-xs" onclick="ladeProfilInRechner()">Aus Profil laden</button>
        </div>
        <div class="grid-2 mt-sm">
  ...
  el.innerHTML = `
    ...
    <div class="scroll-body"><div class="p-main flex-col gap-lg">
      <div class="card card-terra">
        ...
        <div class="mt-sm">
          <label class="lbl">Material des Original-Garns</label>
          <select id="garn-material-orig" onchange="calcGarn()">
            <option value="">— falls bekannt —</option>
            ${Object.keys(MATERIAL_FAKTOREN).map(m => `<option value="${m}">${m}</option>`).join('')}
          </select>
        </div>
      ...
      <div class="card card-green">
        ...
        <div class="mt-sm">
          ...
          <p id="garn-mat-note" class="txt-light mt-sm" style="font-size:0.72rem;"></p>
        </div>
      </div>
...
function calcGarn() {
  ...
  const matOrigKey = document.getElementById('garn-material-orig')?.value;
  const matKey     = document.getElementById('garn-material')?.value;

  const matOrigF = (matOrigKey && MATERIAL_FAKTOREN[matOrigKey]) || {breite:1,hoehe:1,note:''};
  const matF     = (matKey && MATERIAL_FAKTOREN[matKey])         || {breite:1,hoehe:1,note:''};

  const noteEl = document.getElementById('garn-mat-note');
  ...
  if (ol>0 && ok>0 && nl>0) {
    ...
    // Materialkorrektur: Verhältnis Original zu Substitution
    // z.B. Baumwolle (hoehe 1.06) statt Merino (hoehe 1.0) → +6% Verbrauch
    const matKorrBreite = matF.breite / matOrigF.breite;
    const matKorrHoehe  = matF.hoehe  / matOrigF.hoehe;
    const matKorr = (matKorrBreite + matKorrHoehe) / 2;

    const neededM = totalM * matKorr;
    const neededG = Math.ceil(neededM / nl * 100);
    const knauel  = Math.ceil(neededG / ng);

    let matHinweis = '';
    if (matKorr !== 1 && (matOrigKey || matKey)) {
      matHinweis = `<p style="font-size:0.72rem;color:var(--green);margin-top:0.3rem;">Materialkorrekturfaktor: ×${matKorr.toFixed(3)}</p>`;
    }
    ...
    el.innerHTML = `<label class="lbl">Benötigte Menge</label>
      ...
      <p class="txt-light mt-sm" style="font-family:var(--mono);font-size:0.75rem;">Meter gesamt: ${Math.round(neededM)} m</p>
      ${matHinweis}
      <p style="font-size:0.72rem;color:var(--green);margin-top:0.2rem;">+10% Reserve: <strong>${Math.ceil(knauel*1.1)}</strong> Knäuel empfohlen</p>`;
  } else {
...
// ── RAGLAN-PASSEN-RECHNER ─────────────────────────────────────
function renderCalcRaglan() {
  const el = document.getElementById('screen-calc-raglan');
  if (!el) return;

  const maschenarten = Object.keys(RAGLAN_K);

  el.innerHTML = `
    <div class="editor-header">
      <button class="back-btn" onclick="goBack()"><svg width="20" height="20" fill="none" viewBox="0 0 24 24"><polyline points="15 18 9 12 15 6" stroke="currentColor" stroke-width="2" stroke-linecap="round"/></svg></button>
      <h2 style="font-family:var(--serif);font-size:1.1rem;color:var(--text);font-weight:600;">Raglan-Passen-Rechner</h2>
    </div>
    <div class="scroll-body"><div class="p-main flex-col gap-lg">

      <div class="card card-pale-green">
        <p class="txt-light" style="font-size:0.82rem;">Berechnet die <strong style="color:var(--green);">tatsächliche Passen-Höhe</strong> aus Runden — mit einem Korrekturfaktor der Maschenart und Richtung. Belegt durch 6 echte Anleitungen.</p>
      </div>

      <div class="card card-terra">
        <label class="lbl">Konstruktion</label>
        <div class="grid-2 mt-sm">
          <div>
            <label class="lbl">Richtung</label>
            <select id="rag-richtung" onchange="calcRaglan()">
              <option value="top-down">Top-Down (Hals → Körper)</option>
              <option value="bottom-up">Bottom-Up (Körper → Hals)</option>
            </select>
          </div>
          <div>
            <label class="lbl">Hauptmaschenart</label>
            <select id="rag-maschen" onchange="calcRaglan()">
              ${maschenarten.map(m => `<option value="${m}">${m}</option>`).join('')}
            </select>
          </div>
        </div>
        <div id="rag-k-info" class="mt-sm" style="font-size:0.72rem;font-family:var(--mono);color:var(--textLight);"></div>
      </div>

      <div class="card card-green">
        <label class="lbl">Maschenprobe</label>
        <div class="grid-2 mt-sm">
          <div><label class="lbl">Maschen / 10 cm</label><input type="number" id="rag-m" placeholder="18" inputmode="decimal" oninput="calcRaglan()"></div>
          <div><label class="lbl">Reihen / 10 cm</label><input type="number" id="rag-r" placeholder="14" inputmode="decimal" oninput="calcRaglan()"></div>
        </div>
      </div>

      <div class="card">
        <label class="lbl">Passe</label>
        <div class="grid-2 mt-sm">
          <div><label class="lbl">Passen-Runden</label><input type="number" id="rag-runden" placeholder="z.B. 16" inputmode="numeric" oninput="calcRaglan()"></div>
          <div><label class="lbl">Hals-Anschlag (M)</label><input type="number" id="rag-hals" placeholder="z.B. 80" inputmode="numeric" oninput="calcRaglan()"></div>
        </div>
        <div class="grid-2 mt-sm">
          <div><label class="lbl">Zunahmen pro Runde</label><input type="number" id="rag-zun" placeholder="z.B. 8" inputmode="numeric" oninput="calcRaglan()"></div>
          <div><label class="lbl">Bündchen-Runden (optional)</label><input type="number" id="rag-bund" placeholder="0" inputmode="numeric" oninput="calcRaglan()"></div>
        </div>
      </div>

      <div class="card">
        <label class="lbl">Körper nach der Passe</label>
        <div><label class="lbl">Körper-Runden (nach Ärmel-Trennung)</label><input type="number" id="rag-koerper" placeholder="z.B. 30" inputmode="numeric" oninput="calcRaglan()"></div>
      </div>

      <div id="rag-res" class="result-box"><label class="lbl">Ergebnis</label><p class="txt-light">Werte eingeben →</p></div>

      <div class="card" style="background:var(--bgWarm);border:none;">
        <label class="lbl" style="font-size:0.72rem;color:var(--textLight);">k-Faktor Quelle</label>
        <p class="txt-light" style="font-size:0.71rem;">Empirisch aus Hobbii-Anleitungen: Flieder Pulloverkleid (hStb/top-down ±0.5cm), Finisterre (erwfM/top-down ±1.0cm), Katia Raglan (hStb/bottom-up ±0.3cm). Stäbchen-Werte theoretisch aus Stichhöhenverhältnissen (Projektdatei). Toleranz: ±2cm.</p>
      </div>
...
function calcRaglan() {
  const richtung = document.getElementById('rag-richtung')?.value || 'top-down';
  const maschen  = document.getElementById('rag-maschen')?.value  || 'Halbe Stäbchen';
  const mPro10   = +document.getElementById('rag-m')?.value;
  const rPro10   = +document.getElementById('rag-r')?.value;
  const runden   = +document.getElementById('rag-runden')?.value;
  const halsM    = +document.getElementById('rag-hals')?.value;
  const zun      = +document.getElementById('rag-zun')?.value || 8;
  const bundR    = +document.getElementById('rag-bund')?.value || 0;
  const koerperR = +document.getElementById('rag-koerper')?.value || 0;

  // k-Info anzeigen
  const kInfoEl = document.getElementById('rag-k-info');
  const k = RAGLAN_K[maschen]?.[richtung] || 1.0;
  const beleg = (maschen === 'Halbe Stäbchen' && richtung === 'top-down')  ? '(belegt: Flieder ±0.5cm)' :
                (maschen === 'erwfM'           && richtung === 'top-down')  ? '(belegt: Finisterre ±1.0cm)' :
                (maschen === 'Halbe Stäbchen' && richtung === 'bottom-up') ? '(belegt: Katia ±0.3cm)' : '(theoretisch)';
  if (kInfoEl) kInfoEl.textContent = `k = ${k.toFixed(2)} ${beleg}`;

  const el = document.getElementById('rag-res');
  if (!el) return;

  if (!rPro10 || !runden) {
    el.innerHTML = '<label class="lbl">Ergebnis</label><p class="txt-light">Reihen/10cm und Passen-Runden eingeben →</p>';
    return;
  }

  // Berechnungen
  const rProCm       = rPro10 / 10;
  const rHoehe_roh   = runden / rProCm;          // theoretische Passen-Höhe ohne k
  const rHoehe_k     = Math.round(rHoehe_roh * k * 10) / 10;  // mit k-Faktor

  // Halsumfang → Durchmesser
  let halsInfo = '';
  if (mPro10 > 0 && halsM > 0) {
    const halsUmfang = Math.round(halsM / (mPro10/10) * 10) / 10;
    const halsDm     = Math.round(halsUmfang / Math.PI * 10) / 10;
    halsInfo = `<p style="font-family:var(--mono);font-size:0.78rem;color:var(--textLight);">Hals: ${halsM}M → ${halsUmfang}cm Umfang → Ø ${halsDm}cm</p>`;
  }

  // Brustweite nach Passe
  let brustInfo = '';
  if (mPro10 > 0 && halsM > 0) {
    const endMaschen = halsM + (runden * zun);
    const brustUmfang = Math.round(endMaschen / (mPro10/10) * 10) / 10;
    brustInfo = `<p style="font-family:var(--mono);font-size:0.78rem;color:var(--textLight);">Brust nach Passe: ~${endMaschen}M → ~${brustUmfang}cm Umfang</p>`;
  }

  // Bündchen-Höhe (fM ≈ 75% von Grundmasche)
  const fM_ratio = 0.75;
  const bundH = bundR > 0 ? Math.round(bundR / rProCm * fM_ratio * 10) / 10 : 0;

  // Körper-Höhe
  const koerperH = koerperR > 0 ? Math.round(koerperR / rProCm * 10) / 10 : 0;

  // Gesamtlänge
  const gesamt = Math.round((rHoehe_k + koerperH + bundH) * 10) / 10;

  // Toleranz-Klasse
  const tolKlasse = (maschen === 'Stäbchen (dc)' || maschen === 'Doppelstäbchen') ? '±3cm' : '±2cm';

  el.innerHTML = `<label class="lbl">Ergebnis</label>
    <div class="grid-2 mt-sm">
      <div><p class="result-num">${rHoehe_k}</p><p class="result-unit">Passen-Höhe (cm)</p></div>
      ${koerperH > 0 ? `<div><p class="result-num">${koerperH}</p><p class="result-unit">Körper-Höhe (cm)</p></div>` : ''}
      ${bundH > 0    ? `<div><p class="result-num">${bundH}</p><p class="result-unit">Bündchen (cm)</p></div>` : ''}
      ${gesamt !== rHoehe_k ? `<div><p class="result-num" style="color:var(--terra);">${gesamt}</p><p class="result-unit">Gesamtlänge (cm)</p></div>` : ''}
    </div>
    <div class="mt-sm" style="background:var(--bgWarm);border-radius:var(--r-sm);padding:0.65rem;">
      <p style="font-size:0.72rem;font-family:var(--mono);color:var(--textLight);">k=${k.toFixed(2)} · ${runden}R × ${(1/rProCm).toFixed(3)}cm/R = ${rHoehe_roh.toFixed(1)}cm roh → ×k = ${rHoehe_k}cm</p>
      <p style="font-size:0.72rem;color:var(--textLight);margin-top:0.2rem;">Toleranz: ${tolKlasse} — immer anprobieren!</p>
    </div>
    ${halsInfo}${brustInfo}`;
}
